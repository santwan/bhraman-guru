...
const Navbar = () => {
  ...
  const navConfig = [
    ...
    { label: "Blog", to: "/blog", isLive: true },
    ...
    <>
      <nav className={`fixed top-0 w-full z-40 transition-all duration-500 
        ...
        <div className="max-w-[1500px] mx-auto px-4 p-10 md:py-8 flex justify-between items-center">
          ...
          {/* Center Nav */}
          <div className={`hidden sm:flex lg:pr-40 space-x-8 pt-4 text-black dark:text-white text-base font-semibold transition-opacity duration-500 ${scrolled ? "opacity-0 pointer-events-none" : "opacity-100"}`}>
          {navConfig.map(({ label, to, isLive }) =>
              to.startsWith("#") ? (
                <a
                  key={label}
                  href={to}
                  className="lg:text-lg relative flex items-center px-4 py-2 rounded-full transition-all duration-200 transform hover:scale-110 hover:shadow-[0_0_15px_4px_rgba(249,199,79,1)]"
                >
                  {label}
                  {isLive && (
